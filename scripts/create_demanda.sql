...
CREATE TABLE demanda.disciplina_aprovada_por_aluno AS (
...
CREATE TABLE demanda.disciplina_remanescente AS ( --TODO: melhorar o nome
  ...
  FROM consulta.aluno AS a
  INNER JOIN consulta.disciplina_matriz AS m ON matriz_id = a.matriz_id
  JOIN demanda.disciplina_aprovada_por_aluno AS apr ON aluno_id = a.aluno_id
...
CREATE TABLE demanda.aluno_disciplina_aprovada AS (
  SELECT aluno_id, disciplina_id
  FROM academico.alunos 
  JOIN academico.inscricoes ON 
)


-- get all matriz disciplines
-- filter already approved (or approved equivalent)
-- Filter 

-- CREATE TABLE demanda.contagem_aluno_por_disciplina (
--   disciplina_id text,
--   contagem_aluno integer,
-- );
